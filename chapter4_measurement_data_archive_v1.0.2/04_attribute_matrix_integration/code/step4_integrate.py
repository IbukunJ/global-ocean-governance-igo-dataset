...
Outputs
-------
- ch04_s4_data_integration_bundle_rebuilt.xlsx (multi-sheet)
...
def main():
    ...
    orphans["orphan_reason"] = np.where(~orphans["attribute_code_final"].isin(attr_codes), "Invalid attribute_code_final", "Missing IGO")
    ...
    bundle_xlsx = os.path.join(args.outdir, "ch04_s4_data_integration_bundle_rebuilt.xlsx")
    ...
    # CSV exports
    cell_long.to_csv(os.path.join(args.outdir,"ch04_s4_baseline_attribute_matrix_long_rebuilt.csv"), index=False)
    wide_named.to_csv(os.path.join(args.outdir,"ch04_s4_baseline_attribute_matrix_wide_rebuilt.csv"), index=False)
    coverage.to_csv(os.path.join(args.outdir,"ch04_s4_coverage_missingness_rebuilt.csv"), index=False)
    integrity.to_csv(os.path.join(args.outdir,"ch04_s4_integrity_checks_rebuilt.csv"), index=False)
    orphans.to_csv(os.path.join(args.outdir,"ch04_s4_orphan_evidence_rebuilt.csv"), index=False)
    run_log.to_csv(os.path.join(args.outdir,"ch04_s4_run_log_rebuilt.csv"), index=False)

    # Optional figure
    if args.make_figure:
        fig_path = os.path.join(args.outdir, "ch04_s4_evidence_volume_by_attribute_rebuilt.png")
